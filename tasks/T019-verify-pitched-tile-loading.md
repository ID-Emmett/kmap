...
在真实目标浏览器中验证 T029 TileStreamingEngine 与 T022 fog-bounded coverage 的正确性、加载连续性、资源预算和交互性能，形成新瓦片路线的高倾角发布判断。旧 Runtime 与 V2 失败证据只作为风险背景，不作为新系统验收模板。
...
- 以 D032/T029 的通用不变量和 T010/T016 的资源/帧时间安全门槛形成 PASS/BLOCKED 结论，并同步项目事实文档。
...
- T029 和 T022 完成版本及各自 Findings；旧 Runtime 与 V2 的失败证据和隔离边界。
...
- 执行 `pnpm check` 和 T029/T022 针对性测试。
...
- 使用 debug cutoff/coverage overlay 或等价采样证明 fogEnd 前 Coverage 完整；记录 `TileStreamingEngine` 的绝对 Tile/CPU/GPU/请求指标，资源和门槛回归只用于安全检查。
...
- T029 未完成或人工验收未接受时，本 Task 不得开始；旧路径与 V2 人工验收失败不构成再次实施旧 Runtime 或 V2 补丁链的前置条件。
