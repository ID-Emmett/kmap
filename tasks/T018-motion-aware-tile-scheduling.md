...
- `TASKS.md`（只读 T018、T021、T023、T028）
- `tasks/T018-motion-aware-tile-scheduling.md`
- `KNOWLEDGE.md`
...
- `docs/ai-sessions/2026-09-12.md`：需要追溯人工验收失败原话时读取。
- `docs/research/tile-retention-display-fog.md`：需要核对旧路径重复请求证据时读取。
- `docs/evidence/T018-browser-regression.json`：只在关闭或复盘旧任务时读取。
...
- 若仅关闭或重定义任务：`pnpm ai:check` 与 `git diff --check`。
- 若人工负责人要求恢复实施：必须先返回决策会话重写 Task Context Packet。

### Stop Conditions

- 需要修改任何源码文件。
- 需要恢复旧 Runtime motion-aware 生产路径。
- 需要继续 T026/T027 V2 补丁链。
...
- 如需改变默认并发或 cache 预算，必须携带 T018 before/after 证据返回决策会话。
...
- 在相同脚本与网络模型下形成 before/after 指标；若 coarse-cover time、取消请求、废弃 Worker 构建和下载浪费均无可解释改善，任务返回决策会话，不以调参宣称完成。
...
- 2026-09-10：人工负责人未接受连续 pan 的实际加载观感，报告 Tile 闪烁、疑似重新加载以及远处从中心向两侧逐步出现。决策会话复核确认该问题不是 T017 mixed-LOD Coverage 缺口，而是 Ready Cache、warm ancestor 和 Display Coverage 生命周期问题。
...
- T023 `TileEngineV2` 已承接本任务的唯一生产 authority，但人工验收仍失败。T018 保持 `BLOCKED`，不在旧 Runtime 上重新实施；其运动中连续调度与公平性由 T025 重新定义，待 T027 通过后由决策会话关闭或标记为被替代。
