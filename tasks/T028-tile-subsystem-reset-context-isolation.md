...
冻结已失败的 TileEngineV2 补丁链，建立瓦片子系统重置方案、旧实现隔离边界、可信开源基准路线和后续实施任务上下文包，确保新实现会话不会被 T018/T021/T023-T027 的失败补丁路径误导。
...
- `TASKS.md`（只读 T018-T028 当前状态和执行顺序）
...
- `docs/research/tile-lod-scheduling.md`：需要核对 MapLibre/deck.gl 调度事实时读取。
...
- 明确的后续任务拆分：至少包含 MapLibre 基准、源码映射、旧实现隔离和新瓦片实现任务草案或任务创建建议。
...
- 规划可信开源基准、源码映射、legacy import guard、逐帧诊断和最终人工验收任务。
...
- T026/T027 不再作为默认下一步。
- 后续瓦片实施任务必须先经过 T028 输出的上下文包和隔离规则。
...
- 运行 `pnpm ai:check`。
- 运行 `git diff --check`。
- 人工负责人阅读 T028 输出后确认是否进入后续 MapLibre 基准与 V3 实施任务。
...
- 需要后续决策会话将 T028 拆成具体实施任务，至少包括 MapLibre 基准、源码映射、旧实现隔离和新瓦片实现。
