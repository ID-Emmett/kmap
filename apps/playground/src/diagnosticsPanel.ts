...
export function createDiagnosticsPanel(map: Map3D): () => void {
  ...
  panel.innerHTML = /*html*/ `
    <header>
      <div><span class="eyebrow">KMAP · <span><strong data-metric="backend" class="backend">—</strong> · <span data-metric="phase">初始化</span></span></span>
  ...
  pitchButton.id = "pitch-benchmark";
  pitchButton.textContent = "大倾角与海面验收";
  panel.querySelector("#rapid-capture")!.after(pitchButton);
  const fields = new Map<string, HTMLElement>();
  const rows: Record<string, readonly [string, string][]> = {
    frame: [
      ["cpu", "CPU 帧 / P95"],
      ["interval", "帧间隔 P95 / 输入 P95"],
      ["draws", "Draw calls / 三角形"],
      ["phases", "选择 / 提交 / 回收 P95"],
    ...
    tile: [
      ...
      ["queue", "请求排队 / 活动"],
      ["requests", "累计请求 / 取消排队"],
      ["requestTime", "请求链路 P95 / HTTP P95"],
      ["network", "HTTP 次数 / 重试 / 取消"],
      ["networkBytes", "MVT 解压字节 / 错误"],
      ["upload", "上传排队 / 活动"],
      ["empty", "空瓦片 / 失败"],
      ["levels", "目标层级 : 数量"],
      ["drawnLevels", "显示来源层级 : 数量"],
      ["selection", "理想 / 限额合并 / 雾剔除"],
      ["demand", "可见 / 回退 / 预取需求"],
    ],
    cache: [
      ["cache", "驻留 / 预热 / 冷缓存"],
      ...
      ["registry", "资源 / 引用 / 释放"],
      ["scene", "场景瓦片 / 对象"],
      ["geometry", "几何 / 纹理 / 渲染器内存"],
      ["pressure", "预算压力"],
    ],
    camera: [
      ["center", "经度 / 纬度"],
      ["view", "Zoom / Bearing / Pitch"],
      ["position", "相机 XYZ（相对原点·米）"],
      ["origin", "原点 Mercator XY（米）"],
      ["viewport", "视口 / DPR"],
      ["cutoff", "相机雾截止 / 层级跨度"],
    ],
  };
  ...
  for (const element of panel.querySelectorAll<HTMLElement>("[data-metric]"))
    fields.set(element.dataset.metric!, element);
  ...
  const timer = setInterval(() => {
    ...
    set("fps", d.frame.fps.toFixed(0));
    set("backend", d.backend.toUpperCase());
    set("phase", t?.idle ? "idle" : (t?.phase ?? "初始化"));
    ...
    set(
      "phases",
      `${ms(t?.phases.plan?.p95)} / ${ms(t?.phases.cover?.p95)} / ${ms(t?.phases.resources?.p95)}`,
    );
    ...
    set(
      "requests",
      `${t?.scheduler?.requestStarts ?? 0} / ${t?.scheduler?.requestCancels ?? 0}`,
    );
    set(
      "requestTime",
      `${ms(t?.requestTime.p95)} / ${ms(t?.network?.latency.p95)}`,
    );
    set(
      "network",
      `${t?.network?.starts ?? 0} / ${t?.network?.retries ?? 0} / ${t?.network?.cancels ?? 0}`,
    );
    set(
      "networkBytes",
      `${mib(t?.network?.bytes)} / ${t?.network?.errors ?? 0}`,
    );
    set("upload", `${t?.upload?.queued ?? 0} / ${t?.upload?.active ?? 0}`);
    set("empty", `${stats.tiles.empty} / ${stats.tiles.failed}`);
    set(
      "levels",
      Object.entries(t?.levels ?? {})
        .map(([z, n]) => `z${z}: ${n}`)
        .join(" · "),
    );
    set(
      "drawnLevels",
      Object.entries(t?.drawnLevels ?? {})
        .map(([z, n]) => `z${z}: ${n}`)
        .join(" · "),
    );
    set(
      "selection",
      `${t?.idealCount ?? 0} / ${t?.budgetReduced ?? 0} / ${t?.selection.fogCulled ?? 0}`,
    );
    set(
      "demand",
      `${t?.demand.visible ?? 0} / ${t?.demand.fallback ?? 0} / ${t?.demand.predicted ?? 0}`,
    );
    set(
      "cache",
      `${cache?.resident ?? 0} / ${cache?.warm ?? 0} / ${cache?.cold ?? 0}`,
    );
    set(
      "hits",
      `${t?.cacheHits ?? 0} / ${t?.cacheMisses ?? 0} / ${((t?.cacheHitRate ?? 0) * 100).toFixed(1)}%`,
    );
    set(
      "entries",
      `${cache?.entries ?? 0} / ${cache?.maxEntries ?? 256} / ${cache?.evictions ?? 0}`,
    );
    set("cpuMemory", `${mib(cache?.cpuBytes)} / ${mib(cache?.maxCpuBytes)}`);
    set("gpuMemory", `${mib(r?.gpuBytes)} / ${mib(r?.maxGpuBytes)}`);
    set(
      "registry",
      `${r?.entries ?? 0} / ${r?.referencedEntries ?? 0} / ${r?.releases ?? 0}`,
    );
    set("scene", `${d.sceneTiles} / ${stats.resources.objects}`);
    set(
      "geometry",
      `${d.memory.geometries ?? 0} / ${d.memory.textures ?? 0} / ${mib(d.memory.total)}`,
    );
    set(
      "pressure",
      [...(cache?.pressureReasons ?? []), ...(r?.pressureReasons ?? [])].join(
        " · ",
      ) || "正常",
    );
    set(
      "center",
      `${d.view.center.lng.toFixed(6)} / ${d.view.center.lat.toFixed(6)}`,
    );
    set(
      "view",
      `${d.view.zoom.toFixed(2)} / ${d.view.bearing.toFixed(1)}° / ${d.view.pitch.toFixed(1)}°`,
    );
    set(
      "position",
      Object.values(d.camera.position)
        .map((n) => n.toFixed(1))
        .join(" / "),
    );
    set(
      "origin",
      `${d.camera.origin.meters.x.toFixed(1)} / ${d.camera.origin.meters.y.toFixed(1)}`,
    );
    set(
      "viewport",
      `${d.viewport.width} × ${d.viewport.height} / ${d.viewport.pixelRatio}`,
    );
    set(
      "cutoff",
    ...
    chart.strokeStyle = "#d8e2dc";
    chart.beginPath();
    chart.moveTo(0, 22);
    chart.lineTo(320, 22);
    chart.stroke();
    chart.strokeStyle = "#1a866b";
    chart.lineWidth = 1.5;
    chart.beginPath();
    history.forEach((v, i) => {
      const x = (i / 119) * 320;
      const y = 42 - Math.min(40, v);
  ...
  panel.addEventListener("click", (event) => {
    ...
    if (button.id === "export") {
      requestAnimationFrame(() => {
        const value = {
          screenshot: document
            .querySelector<HTMLCanvasElement>("#map-canvas")
            ?.toDataURL("image/png"),
          at: new Date().toISOString(),
          diagnostics: latest,
          timeline: map.getTileTimeline(),
          benchmark: document.documentElement.dataset.kmapBenchmark
            ? JSON.parse(document.documentElement.dataset.kmapBenchmark)
            : undefined,
        };
        if (import.meta.env.DEV) {
          void fetch("/__kmap/diagnostics", {
            method: "POST",
            ...
            .catch(() => {
              download(value);
              status.textContent = "诊断 JSON 已下载";
            });
        } else download(value);
      });
      return;
    }
    if (button.id === "tile-labels") {
      const enabled = overlay.toggle();
      button.setAttribute("aria-pressed", String(enabled));
      button.textContent = enabled ? "隐藏瓦片编号" : "显示瓦片编号";
      return;
    }
    if (running) return;
    if (button.id === "pitch-benchmark") {
      ...
      void import("./pitchBenchmark.js")
        .then(({ runPitchBenchmark }) =>
          runPitchBenchmark(map, (text) => {
    ...
    if (button.id === "raf-baseline") {
      running = true;
      button.disabled = true;
      void measureRafBaseline(map, (text) => {
        status.textContent = text;
      })
        .catch((error) => {
          status.textContent = String(error);
    ...
    if (button.id === "city-flight" || button.id === "fly-guangzhou") {
      ...
      const flight = async () => {
        if (button.id === "city-flight") {
          ...
          status.textContent = "城市飞行：广州";
          await flyToCity(map, CITIES.guangzhou, 10000, () => stopped);
        }
    ...
    if (button.id === "rapid-capture") {
      window.location.assign("/?capture=rapid");
      return;
    }
    if (button.dataset.pitch !== undefined)
      map.setView({ pitch: Number(button.dataset.pitch) });
    ...
    if (button.dataset.view === "rotate")
      map.setView({ bearing: view.bearing + 45 });
    if (button.id === "benchmark" || button.id === "fps-benchmark") {
      running = true;
      button.disabled = true;
      void runBrowserBenchmark(
        map,
        (message) => {
        ...
        .then((result) => {
          document.documentElement.dataset.kmapBenchmark =
...
function download(value: unknown): void {
  ...
  anchor.href = url;
  anchor.download = `kmap-webgpu-${Date.now()}.json`;
  anchor.click();
