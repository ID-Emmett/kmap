...
describe('地名层级与默认配色', () => {
  it('真实 KYE 数据依次显示国家、省名、带红圈的省会及普通城市', () => {
    ...
    expect(at(3).map(l => l.text)).toEqual(['中华人民共和国']);
    expect(at(5).some(l => l.text === '河北')).toBe(true);
    expect(at(5).every(l => l.layerId === 'province-label')).toBe(true);
    expect(at(6).find(l => l.text === '北京')).toMatchObject({ layerId: 'capital-label', icon: 'capital', iconColor: '#d3424b' });
