...
- 全缩放球面地图、文字/图标配置、三套主题与双后端性能的 141 项测试和 30 场景数据见 [地图体验验证](../evidence/map-experience/README.md)。

- 最大倾角为 75°。CPU 与 TSL 使用一致的球面距离雾约束，球缘薄雾作用于地平线。目标集合保持单一层级，向下/向上 zoom 滞回分别为 0.18/0.08。
- 面、线、建筑和文字使用连续球面坐标，pan 经度可连续跨越完整一周。文字候选采用可见瓦片预算分配、保留缓存和小位移节流；九类 SDF 图标共用字形 atlas。
- 晴昼、深海、霓彩覆盖地图面、线、边界、建筑、文字、图标、雾与背景，uniform palette 占 2 KiB。主题切换复用瓦片几何，palette 内存独立报告。
...
- 2026-09-17 的 45 秒连续交互：WebGL2 163.58 FPS、CPU P95 6.9 ms；WebGPU 164.85 FPS、CPU P95 7.1 ms。两者帧间隔 P95 均为 6 ms，覆盖缺口、空目标、运行错误均为 0。
- 两个后端各 15 个静态场景全部收敛，各 197 张图像完成模型视觉检查。独立 12 秒运动截图与 45 秒性能采样分别保存。
