...
describe('阶段队列的可见需求优先与过期释放', () => {
  ...
  it('面几何从上传产物转为驻留资源后字节守恒，并服从 GPU 预算', async () => {
    ...
    const gpuPerTile = fillBytes(fills);
    ...
    const pendingCompiles: (() => void)[] = [];
    const compileAsync = vi.fn(() => new Promise<void>(resolve => { pendingCompiles.push(resolve); }));
    const finish = () => pendingCompiles.shift()!();
    ...
    for (const e of [a, b]) { store.setState(e, 'upload'); e.result = { id: e.address.x, bitmap: { width: 1, height: 1, close: closed } as unknown as ImageBitmap, fills, features: 1, paintMs: 1, empty: false }; store.refreshBytes(e); }
    expect(resultBytes(a.result)).toBe(4 + fillBytes(fills));
    const cpuBefore = store.cpuBytes;
    pipeline.upload(10, camera);
    // 锚点纹理由 CPU 侧位图承担，不再创建与上传 GPU 纹理。
    expect(initTexture).not.toHaveBeenCalled(); expect(compileAsync).toHaveBeenCalledOnce();
    expect(a.state).toBe('preparing'); expect(store.available.has(a.key)).toBe(false); expect(a.result).toBeUndefined();
    ...
    // GPU 预算只容纳一个完整来源，第二个来源继续等待容量而不是被编译门闸阻塞。
    expect(b.state).toBe('upload'); expect(compileAsync).toHaveBeenCalledOnce(); expect(closed).not.toHaveBeenCalled();
  ...
  it('编译在途不阻塞下一个来源进入准备，并发上限由 prepares 约束', () => {
    const surfaces = new TileSurfaces(new Scene(), new Color('#ffffff'));
    const fills = { positions: new Float32Array(9), colors: new Float32Array(9), styles: new Float32Array(9), indices: new Uint32Array([0, 1, 2]) };
    const store = new TileStore(surfaces, {}, new Set());
    const closed = vi.fn(), camera = new PerspectiveCamera();
    const compileAsync = vi.fn(() => new Promise<void>(() => {}));
    const pipeline = new TilePipeline(store, { canvas: {} as HTMLCanvasElement, source: { id: 'fixture', tiles: ['/{z}/{x}/{y}'], minZoom: 0, maxZoom: 17 }, layers: [] },
      { initTexture: vi.fn(), compileAsync } as unknown as WebGPURenderer, '#ffffff', () => {}, () => {}, () => {});
    const entries = [0, 1, 2].map(x => {
      const entry = store.create({ z: 4, x, y: 0 }, 'visible', 0, 0)!;
      store.setState(entry, 'upload');
      entry.result = { id: x, bitmap: { width: 1, height: 1, close: closed } as unknown as ImageBitmap, fills, features: 1, paintMs: 1, empty: false };
      store.refreshBytes(entry);
      return entry;
    });
    pipeline.upload(10, camera); pipeline.upload(11, camera);
    expect(entries[0]!.state).toBe('preparing'); expect(entries[1]!.state).toBe('preparing');
    pipeline.upload(12, camera);
    expect(entries[2]!.state).toBe('upload'); expect(compileAsync).toHaveBeenCalledTimes(2);
    expect(closed).not.toHaveBeenCalled();
    pipeline.dispose(); store.dispose(); surfaces.dispose();
  });
  it('八个过期上传产物在回访保护窗口之后释放，当前解码瓦片获得 Worker 槽位', () => {
    const surfaces = new TileSurfaces(new Scene(), new Color('#ffffff'));
    const store = new TileStore(surfaces, {}, new Set());
    const closed = vi.fn();
    // 创建时间早于回访保护窗口，超出窗口的过期产物恢复立即释放语义。
    for (let i = 0; i < 8; i++) {
      const e = store.create({ z: 4, x: i, y: 0 }, 'predicted', Infinity, -10000)!;
      store.setState(e, 'upload'); e.result = { id: i, bitmap: { width: 256, height: 256, close: closed } as unknown as ImageBitmap, features: 0, paintMs: 1, empty: false }; store.refreshBytes(e);
    }
    const visible = store.create({ z: 4, x: 10, y: 0 }, 'visible', 0, 0)!;
    store.setState(visible, 'decoded'); visible.buffer = new ArrayBuffer(1); store.refreshBytes(visible);
