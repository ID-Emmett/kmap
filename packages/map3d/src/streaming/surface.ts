...
export class TileSurfaces {
  ...
  create(bitmap: ImageBitmap, address: Address) {
    ...
    mesh.frustumCulled = false; mesh.visible = false; mesh.renderOrder = address.z;
    this.scene.add(mesh);
    return { mesh, map, bitmap, bytes: Math.ceil(bitmap.width * bitmap.height * 4 * 4 / 3) };
  }
