...
export function createLineState(data: LineData) {
  ...
  return { pixelScale: { value: 1 / 256 }, viewZoom: { value: -1 }, widths, dashes, data };
}

export function updateLineState(state: ReturnType<typeof createLineState>, address: Address, zoom: number): void {
  if (state.viewZoom.value === zoom) return;
  state.viewZoom.value = zoom; state.pixelScale.value = linePixelScale(address.z, zoom);
  ...
  state.data.paints.forEach((paint, i) => {
    state.widths[i * 4] = lineWidth(paint, zoom) * (paint.widthUnit === 'pixels' ? state.pixelScale.value : metersToTile);
  });
}
