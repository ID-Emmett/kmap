...
scope.onmessage = ({ data }) => {
  ...
  try {
    ...
    scope.postMessage({ id: data.id, bitmap, lines, fills, buildings, features: fills.features + lines.features + buildings.features, paintMs: performance.now() - started, empty: data.buffer.byteLength === 0 },
