...
export function tileDiagnostics(engine: StreamingEngine) {
  ...
  return {
    ...
    cache: { resident, warm: ready - resident, cold: 0, entries: entries.length, maxEntries: engine.maxEntries, evictions: engine.evictions,
      cpuBytes: engine.gpuBytes * .75, maxCpuBytes: engine.maxCpuBytes, pressure: engine.gpuBytes * .75 > engine.maxCpuBytes || entries.length > engine.maxEntries, pressureReasons: engine.gpuBytes * .75 > engine.maxCpuBytes ? ['CPU budget'] : [] },
