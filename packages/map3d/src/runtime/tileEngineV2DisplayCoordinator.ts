...
import {
  ...
  isAncestorReplacementComplete,
} from './tileEngineV2DisplayCoverage.js';

const DEFAULT_TRANSITION_MS = 180;
const FALLBACK_WARM_ANCESTOR_LEVELS = 2;

export interface TileEngineV2DisplayRecord {
  id: string;
...
export interface TileEngineV2DisplaySelection {
  recordId: string;
...
export interface TileEngineV2DisplayPlan {
  token: number;
  selections: readonly TileEngineV2DisplaySelection[];
  requestedFallbacks: readonly RenderTileKey[];
  requiredFallbacks: readonly RenderTileKey[];
  animating: boolean;
}

interface MutableV2Selection {
  recordId: string;
  keys: Map<string, RenderTileKey>;
  opacity: number;
  role: TileEngineV2DisplaySelection['role'];
  transitionStartedAt: number | undefined;
}

/** 在 Target Coverage 与实际显示资源之间协调 fallback、过渡和 display pin。 */
export class TileEngineV2DisplayCoordinator {
  readonly #durationMs: number;
  readonly #reducedMotion: boolean;
  readonly #minZoom: number;
  #targetSignature = '';
  #targetToken = 0;
  #active = new Map<string, MutableV2Selection>();
  #outgoing = new Map<string, MutableV2Selection>();
  #transitionNow = 0;
  ...
  advance(now: number): TileEngineV2DisplayPlan {
    this.#transitionNow = now;
    this.#applyTransitionOpacity(now);
  ...
  #selectTargets(
    targets: readonly TileCoverageEntry[],
    records: ReadonlyMap<string, TileEngineV2DisplayRecord>,
  ): Map<string, MutableV2Selection> {
    const selections = new Map<string, MutableV2Selection>();
    ...
    for (const target of targets) {
      ...
      if (
        ancestor !== undefined &&
        !isAncestorReplacementComplete(ancestor.renderKey, targets, records)
      ) {
    ...
    if (!hasCompleteTargetCoverage(selections.values(), targets)) {
      return new Map();
    }
    return selections;
  }
  ...
  #preserveOutgoing(
    next: ReadonlyMap<string, MutableV2Selection>,
    targets: readonly TileCoverageEntry[],
  ): void {
  ...
  #removeOverlappingOutgoing(next: ReadonlyMap<string, MutableV2Selection>): void {
    const nextKeys = flattenSelectionKeys(next.values());
    for (const [recordId, selection] of this.#outgoing) {
...
function addSelectionKey(
  selections: Map<string, MutableV2Selection>,
  recordId: string,
  key: RenderTileKey,
  role: MutableV2Selection['role'],
): void {
