...
import { DisplayCoverageCoordinator } from './displayCoverage.js';
import type {
  DisplayCoveragePlan,
  DisplayCoverageRecord,
} from './displayCoverage.js';
...
export class TileRuntimeDisplayBridge<Payload> {
  readonly #coordinator: DisplayCoverageCoordinator;
  ...
  constructor(reducedMotion: boolean, useRealClock: boolean, minZoom: number) {
    this.#coordinator = new DisplayCoverageCoordinator({ reducedMotion, minZoom });
  ...
  #getCandidates(
    records: ReadonlyMap<string, TileRecord<Payload>>,
  ): Map<string, DisplayCoverageRecord> {
    const candidates = new Map<string, DisplayCoverageRecord>();
    for (const record of records.values()) {
      candidates.set(record.id, {
        id: record.id,
        key: record.key,
        ready: record.resource !== undefined && record.state === 'ready',
        fallbackEligible:
