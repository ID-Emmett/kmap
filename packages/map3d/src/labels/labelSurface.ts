...
import { Fn, attribute, cameraProjectionMatrix, cameraViewMatrix, float, fwidth, max, mix, smoothstep, texture, uniform, uv, varying, vec2, vec4 } from 'three/tsl';
import { GLYPH_RANGE } from './glyphs.js';
import type { GlyphAtlas } from './glyphAtlas.js';

export interface GlyphQuad { x: number; y: number; left: number; top: number; width: number; height: number; u: number; v: number; du: number; dv: number; angle: number; color: string | number; haloColor: string | number; haloWidth: number; scale: number; born: number }
...
export class LabelSurface {
  ...
  constructor(atlas: GlyphAtlas) {
    ...
    for (const [name, size] of [['labelAnchor', 2], ['labelRect', 4], ['labelUV', 4], ['labelStyle', 4], ['labelColor', 3], ['labelHalo', 3]] as const)
    ...
    material.vertexNode = Fn(() => {
      const clip = cameraProjectionMatrix.mul(cameraViewMatrix).mul(vec4(anchor.x.sub(this.origin.x), 0, anchor.y.sub(this.origin.z), 1)).toVar();
      const pixel = vec2(rect.x.add(uv().x.mul(rect.z)), rect.y.add(float(1).sub(uv().y).mul(rect.w)));
      const c = style.x.cos(), s = style.x.sin();
  ...
  write(quads: readonly GlyphQuad[]): void {
    ...
    for (let i = 0; i < this.count; i++) {
      ...
      set('labelAnchor', [q.x, q.y]); set('labelRect', [q.left, q.top, q.width, q.height]); set('labelUV', [q.u, q.v, q.du, q.dv]);
